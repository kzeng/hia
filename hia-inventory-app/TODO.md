...
测试发现ftp上传DCIM 文件夹下的 pic文件夹时， 并没有上传pic这个层级完整文件夹，而是上传pic下的文件夹。 我的预期是 上传DCIM 下的 完整pic文件夹到ftp server.

由于网络原因或者别的意外， ftp upload 可能会卡住， 希望提供一个 强制终止退出ftp upload 的按钮， 这样用户可以有机会重新连接上传，否则会卡在ftp upload 无法退出。
...
# ISSUES
1. 提示："删除失败: 请检查应用是否有文件删除权限，或文件夹是否为空"， 实际上文件夹存在且不为空。 检查代码， 找到最有可能的原因。
补充信息： 系统为Android 13 (API 33), 我通过系统的文件管理器可以手工删除文件夹和文件夹下文件。

最可能原因
- Android 13 作用域存储限制：ContentResolver.delete 删除“不是本应用创建”的媒体会抛 SecurityException，需要 MediaStore.createDeleteRequest 触发系统授权对话框。你捕获异常后返回 false，导致提示“删除失败…”。系统文件管理器能删，是因为它有特权。
- 查询条件过严：RELATIVE_PATH 使用“=”，无法覆盖子目录或结尾斜杠差异，可能剩余文件没删干净。

修复要点
- 改为 LIKE 查询，覆盖子目录。
- 在 Q+ 上当直接删除失败时，收集该目录下所有图片/视频的 Uri，调用 MediaStore.createDeleteRequest 发起系统授权删除。
